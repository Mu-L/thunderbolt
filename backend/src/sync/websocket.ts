...
import { checkMigrationVersion, getServerVersion, pullChanges, pushChanges, type SerializedChange } from './sync-core'
import { upsertDevice } from './utils'
    ...
    async message(ws, rawMessage) {
      ...
      try {
        if (message.type === 'auth') {
          ...
          // Check migration version using core function
          const requiredVersion = await checkMigrationVersion(database, authUser.id, migrationVersion)
          ...
          // Get current server version using core function
        ...
        if (message.type === 'push') {
          // Handle push changes using core function
          ...
          const result = await pushChanges(
            {
              ...
              migrationVersion: client.migrationVersion,
            },
            changes,
          )
          ...
          client.lastServerVersion = BigInt(result.serverVersion)
        ...
        if (message.type === 'pull') {
          // Handle pull changes using core function
